...
// ═══════════════════════════════════════════════════════════════
//  ROT + KPT ALGORITHM  (exact port of rot_kpt_engine.py)
// ═══════════════════════════════════════════════════════════════

function runEngine(groups, totalBudget, floorFraction = 0.8, minStops = 30) {
  if (!groups.length || totalBudget <= 0) return [];

  // Split: engine-eligible groups have a real population denominator (pop>=100).
  // Display-only groups (pop<100 but stops>0) are shown in results but excluded
  // from ROT/KPT/disproportion calculations — dividing by a near-zero population
  // share would produce meaningless disproportion figures in the thousands.
  const engineGroups  = groups.filter(g => (g.population || 0) >= 100);
  const displayGroups = groups.filter(g => (g.population || 0) < 100 && (g.stops || 0) > 0);
  ...
  const areaAvgHR  = totalStops > 0 ? totalHits / totalStops : 0;
  ...
  const rotFlagged = g => {
    ...
    return hasData && flagDec && flagOut;
  };
  ...
  engineGroups.forEach(g => {
    if (rotFlagged(g) && areaAvgHR > 0 && totalStops > 0) {
      const fairSS = (g.hits / areaAvgHR) / totalStops;
      const ex = Math.max(0, Math.min((stopShare(g) - fairSS) * totalBudget, popProp(g)));
      excessQuota[g.name] = ex;
      totalPool += ex;
    } else {
      excessQuota[g.name] = 0;
    }
  });

  // Stage 3 — Redistribute
  const unflagged = engineGroups.filter(g => !rotFlagged(g));
  const unflaggedPopSum = unflagged.reduce((s, g) => s + popShare(g), 0);

  const rawQuota = {};
  engineGroups.forEach(g => {
    if (rotFlagged(g)) {
      rawQuota[g.name] = popProp(g) - excessQuota[g.name];
    } else {
      const gain = unflaggedPopSum > 0 ? totalPool * (popShare(g) / unflaggedPopSum) : 0;
      // Base: always use the current stop share, regardless of whether pool > 0.
      // Only the gain (redistributed excess from flagged groups) is added on top.
      // Using popProp(g) as base was silently reallocating unflagged over-stopped groups
      // (e.g. Black at 4x with above-average HR) to population-proportional without flagging.
      const base = stopShare(g) * totalBudget;
      rawQuota[g.name] = base + gain;
    }
  });

  // Stage 4 — Floor + donor deduction
  // Floor only activates when a reallocation is in progress (pool > 0).
  // When nothing is flagged, applying the floor would create spurious movement
  // by lifting under-stopped groups toward population-proportional even though
  // no excess has been freed to fund that movement.
  const floorQ = g => totalPool > 0 ? floorFraction * popProp(g) : 0;
  const topups = {};
  let totalTopup = 0;
  engineGroups.forEach(g => {
    const tu = Math.max(0, floorQ(g) - rawQuota[g.name]);
    topups[g.name] = tu;
    totalTopup += tu;
  });

  const donors = engineGroups.filter(g => !rotFlagged(g) && rawQuota[g.name] > floorQ(g));
  const donorSum = donors.reduce((s, g) => s + (rawQuota[g.name] - floorQ(g)), 0);

  const afterFloor = {};
  engineGroups.forEach(g => {
    if (topups[g.name] > 0) {
      afterFloor[g.name] = floorQ(g);
    } else if (donors.find(d => d.name === g.name) && donorSum > 0) {
      const shareOfDeduction = (rawQuota[g.name] - floorQ(g)) / donorSum;
      afterFloor[g.name] = rawQuota[g.name] - totalTopup * shareOfDeduction;
    } else {
      afterFloor[g.name] = rawQuota[g.name];
    }
  });

  // Stage 5 — Integer allocation (largest remainder)
  const floored    = {};
  const remainders = {};
  engineGroups.forEach(g => {
    floored[g.name]    = Math.floor(afterFloor[g.name]);
    remainders[g.name] = afterFloor[g.name] - floored[g.name];
  });

  const deficit = totalBudget - engineGroups.reduce((s, g) => s + floored[g.name], 0);
  const byRem   = [...engineGroups].sort((a, b) => remainders[b.name] - remainders[a.name]);
  const finalQ  = { ...floored };
  byRem.forEach((g, i) => { if (i < deficit) finalQ[g.name] += 1; });

  // Assemble engine groups with full results
  const engineResults = engineGroups.map(g => {
    ...
    return {
      ...
      floorQuota: floorQ(g), afterFloor: afterFloor[g.name],
      finalQuota: fq, perPersonRate: fq / g.population,
      disproportion: ps > 0 ? (totalPool > 0 ? (fq / totalBudget) / ps : ss / ps) : 0,
      floorEnforced: topups[g.name] > 0, totalPool,
      displayOnly: false,
    };
  });
...
function ResultCard({ r, index, maxQuota, areaAvgHR }) {
  ...
  return (
    <div style={{
      ...
      {r.displayOnly && (
        <div style={{ position: "absolute", top: 10, right: 12, fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: "0.08em", textTransform: "uppercase" }}>
          ℹ no pop. estimate
        </div>
      )}
      {r.rotFlagged && (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: C.sub }}>
            {r.displayOnly ? "pop. estimate unavailable" : `pop. ${r.population.toLocaleString()}`}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 24, fontWeight: 800, color: r.displayOnly ? C.muted : dispColor, fontFamily: "'DM Mono', monospace", lineHeight: 1 }}>
            {r.displayOnly ? "N/A" : `${r.disproportion.toFixed(2)}×`}
          </div>
        </div>
      </div>
          ...
          {results.map((r, i) => {
            ...
            return (
              <tr key={r.name} style={{ borderBottom: `1px solid ${C.border}22` }}>
                ...
                <td style={{ padding: "9px 12px", fontFamily: "monospace", fontWeight: 700, color: dispColor }}>{r.disproportion == null ? "N/A" : `${r.disproportion.toFixed(2)}×`}</td>
              </tr>
            );
          })}
        </tbody>
...
// ═══════════════════════════════════════════════════════════════
//  HORIZONTAL COMPARISON CHART  (ported from rot_kpt_dashboard)
// ═══════════════════════════════════════════════════════════════
...
export default function Dashboard() {
  ...
  const [minStops,      setMinStops]      = useState(30);
...
;
  ...
  // Each group carries a per-month stops array `s` and hits array `h`, aligned
  // to MONTHS (oldest → newest). "Last N months" = sum of the final N entries,
  // so the window reflects what actually happened in those months (real monthly
  // composition AND hit rate), not an even-spread approximation. Groups with no
  // monthly arrays (e.g. the manual fallback) degrade to a proportional slice.
  const MONTHS = (uploadedData?.months) || [];
  const DATA_SPAN_MONTHS = MONTHS.length || 36;
  // Clamp the requested window to the months we actually have.
  ...
  // Sum the last `effWindow` months for each group (true rolling window).
  ...
  // Share of all recorded stops that fall inside the window (for display).
  ...
  // Annualised rate derived from the FULL 36-month record, not the window.
  // This keeps the base budget identical across all window choices so that
  // comparisons between windows reflect only changes in ROT/KPT proportions
  // and windowAreaHR — not changes in total budget size.
  // The rolling window affects: (a) stop/hit proportions fed to ROT+KPT,
  // and (b) windowAreaHR for volume scaling. It does NOT affect rawBudget.
  const annualRate   = Math.round(totalRecordedStops / (DATA_SPAN_MONTHS / 12));
  const annualBudget = annualRate;   // kept for display
  const rawBudget    = Math.round(annualRate * (budgetPeriod / 12));

  // ── Stage 1: outcome-based volume scaling ────────────────────────────────
  // scale = min(1, area_hr / national_hr) over the chosen rolling window.
  // An area performing at or above the national benchmark is left unscaled;
  // an area persistently below it has its total budget reduced proportionally.
  const windowAreaStops = useMemo(() => windowGroups.reduce((s, g) => s + (g.stops || 0), 0), [windowGroups]);
  const windowAreaHits  = useMemo(() => windowGroups.reduce((s, g) => s + (g.hits  || 0), 0), [windowGroups]);
  const windowAreaHR    = windowAreaStops > 0 ? windowAreaHits / windowAreaStops : 0;
  const volumeScale     = nationalHR > 0 ? Math.min(1, windowAreaHR / nationalHR) : 1;
  ...
  const results = useMemo(
    () => runEngine(windowGroups, totalBudget, floorFrac, minStops),
    [windowGroups, totalBudget, floorFrac, minStops]
  );
  ...
  // ════════════════════════════════════════════════════════════════
  //  PAGE 1 — INPUTS  (rolling window + area only)
  // ════════════════════════════════════════════════════════════════
  if (page === "input") {
    const units = allUnits[geoLevel] || [];
    return (
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "24px" }}>
            ...
            <div style={{ background: "#0D1525", border: `1px solid ${C.border}`, borderRadius: 8, padding: "12px 14px", marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>
                Annualised rate (full 3-yr record): {annualRate.toLocaleString()}/yr
                × ({budgetPeriod}/12) = {rawBudget.toLocaleString()} raw
                {volumeScale < 1
                  ? <> × volume scale {volumeScale.toFixed(3)} (area HR {(windowAreaHR*100).toFixed(1)}% ÷ national {(nationalHR*100).toFixed(1)}%) = <strong style={{ color: C.text }}>{totalBudget.toLocaleString()}</strong></>
                  : <> × volume scale 1.000 (area HR {(windowAreaHR*100).toFixed(1)}% ≥ national {(nationalHR*100).toFixed(1)}%, no reduction) = <strong style={{ color: C.text }}>{totalBudget.toLocaleString()}</strong></>
                }
              </div>
            </div>
  ...
  // ════════════════════════════════════════════════════════════════
  //  PAGE 2 — RESULTS  (overview = non-technical · technical = detail)
  // ════════════════════════════════════════════════════════════════
      ...
      <div style={{ background: `linear-gradient(135deg, #0D1525 0%, #111E35 100%)`, borderBottom: `1px solid ${C.border}`, padding: "16px 20px 0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1, background: C.border, borderRadius: "10px 10px 0 0", overflow: "hidden" }}>
          {[
            ...
            { label: "Pool Freed", value: fmt1(totalPool), color: C.gold },
          ].map(s => (
        ...
        {layer === "technical" && (
          <div style={{ display: "flex", gap: 2, marginTop: 1, background: C.border }}>
            {[{ id: "volume", label: "Stage 1: Volume Scale" }, { id: "rot", label: "Stage 2: ROT" }, { id: "kpt", label: "Stage 3: KPT" }].map(t => (
      ...
      <div style={{ padding: "20px 20px 40px" }}>
          ...
          <div>
                ...
                return (
                  <div key={r.name} style={{ background: C.card, borderRadius: 12, padding: "16px", border: `1px solid ${C.border}`, borderTop: `3px solid ${pal.solid}` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 800, fontSize: 15, color: C.text }}>{r.name}</div>
                        <div style={{ fontSize: 11, color: C.sub }}>pop. {r.population.toLocaleString()}</div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#0D1525", borderRadius: 8, padding: "12px 14px" }}>
                      ...
                      <div style={{ fontSize: 24, fontWeight: 800, color: dispColor, fontFamily: "'DM Mono', monospace" }}>{r.disproportion == null ? "N/A" : `${r.disproportion.toFixed(2)}×`}</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12, marginTop: 18 }}>
              ...
              <HorizChart
                ...
                note="Target = 1.00×"
                rows={results.map((r) => {
                  const before = r.stopShare / (r.popShare || 1);
                  const after = r.disproportion;
                  if (after == null) return null;
                  const dispC = after > 1.3 ? C.red : after < 0.85 ? C.gold : C.green;
                  return { group: r.name, labelA: "Before", valA: `${before.toFixed(2)}×`, a: before, labelB: "After", valB: `${after.toFixed(2)}×`, b: after, bColor: dispC, badge: `${after.toFixed(2)}× engine`, tag: after > 1.3 ? "over" : after < 0.85 ? "under" : null };
                }).filter(Boolean)}
              />
            </div>
        ...
        {layer === "technical" && techTab === "volume" && (
          <Card accent={C.accent}>
            <div style={{ fontWeight: 800, color: C.text, marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>Stage 1 — Outcome-Based Volume Scaling</div>
            <p style={{ margin: "0 0 12px", fontSize: 13, color: C.sub, lineHeight: 1.6 }}>
              The total search budget is scaled by <code>min(1, area HR / national HR)</code> over the chosen rolling window.
              If the area hit rate is below the national benchmark the budget is reduced proportionally; if at or above, no reduction is applied.
              The mechanism is self-correcting: as productivity recovers the scale factor rises back toward 1.0 automatically.
            </p>
            <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, padding: "12px 16px", fontFamily: "'DM Mono', monospace", fontSize: 13, marginBottom: 12 }}>
              <div style={{ marginBottom: 6, color: C.accent, fontWeight: 700 }}>STAGE 1 — OUTCOME-BASED VOLUME SCALE (SETS TOTAL BUDGET)</div>
              <div style={{ color: C.text, lineHeight: 1.8 }}>
                scale = min(1, area_hr / national_hr) = min(1, {(windowAreaHR*100).toFixed(1)}% / {(nationalHR*100).toFixed(1)}%) = <strong style={{ color: volumeScale < 1 ? C.gold : C.green }}>{volumeScale.toFixed(3)}</strong>
              </div>
              <div style={{ color: C.muted, fontSize: 12, marginTop: 6 }}>
                Budget: {rawBudget.toLocaleString()} raw → <strong style={{ color: C.text }}>{totalBudget.toLocaleString()}</strong> after scaling
                {volumeScale < 1
                  ? <span style={{ color: C.gold }}> ({(31835).toLocaleString()} searches removed nationally — 8.0% reduction)</span>
                  : <span style={{ color: C.green }}> (no reduction — at or above national benchmark)</span>
                }
              </div>
            </div>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 180, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px" }}>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>AREA HIT RATE ({effWindow}mo window)</div>
                <div style={{ fontSize: 22, fontWeight: 800, color: windowAreaHR < nationalHR ? C.gold : C.green }}>{(windowAreaHR*100).toFixed(2)}%</div>
              </div>
              <div style={{ flex: 1, minWidth: 180, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px" }}>
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginBottom: 4 }}>NATIONAL BENCHMARK (36mo)</div>
        ...
        {layer === "technical" && techTab === "rot" && (
          <Card accent={C.accent}>
            <div style={{ fontWeight: 800, color: C.text, marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>Stage 2 — Robust Outcome Test</div>
            <p style={{ margin: "0 0 10px", fontSize: 13, color: C.sub, lineHeight: 1.6 }}>Flags a group when both signals fire simultaneously. The dual-signal requirement prevents false positives — a group with a low hit rate but low stop share is under-policed, not over-policed; the ROT correctly ignores it. Note: quotas are aggregate monthly ceilings for each policing unit. Every individual stop still requires reasonable grounds under PACE Section 1.</p>
            ...
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid #243046" }}><div style={{ fontSize: 10, color: "#8896B3", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 10 }}>Sources</div><div style={{ borderLeft: "3px solid #00E5FF", paddingLeft: 10, marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #24304622" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 6px", borderRadius: 4, background: "#00E5FF22", color: "#00E5FF", border: "1px solid #00E5FF44" }}>PRIMARY</span><span style={{ fontSize: 11, fontWeight: 700, color: "#E8EDFB" }}>Gaebel, J. et al. (2025)</span></div><div style={{ fontSize: 11, fontStyle: "italic", color: "#8896B3", marginBottom: 2 }}>A simple, statistically robust test of discrimination in arrest rates</div><div style={{ fontSize: 10, color: "#4A5568", marginBottom: 4 }}>Proceedings of the National Academy of Sciences</div><div style={{ fontSize: 10, color: "#8896B3", lineHeight: 1.55, background: "#0D1525", borderRadius: 5, padding: "5px 8px" }}>Introduces the Robust Outcome Test. The dual-signal requirement (decision-rate AND outcome-rate) provides stronger guarantees than single-signal tests.</div></div><div style={{ borderLeft: "3px solid #FFD166", paddingLeft: 10, marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #24304622" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 6px", borderRadius: 4, background: "#FFD16622", color: "#FFD166", border: "1px solid #FFD16644" }}>FOUNDATIONAL</span><span style={{ fontSize: 11, fontWeight: 700, color: "#E8EDFB" }}>Knowles, J., Persico, N. & Todd, P. (2001)</span></div><div style={{ fontSize: 11, fontStyle: "italic", color: "#8896B3", marginBottom: 2 }}>Racial bias in motor vehicle searches: Theory and evidence</div><div style={{ fontSize: 10, color: "#4A5568", marginBottom: 4 }}>Journal of Political Economy, 109(1), 203–229</div><div style={{ fontSize: 10, color: "#8896B3", lineHeight: 1.55, background: "#0D1525", borderRadius: 5, padding: "5px 8px" }}>Proves that absent discrimination, hit rates equalise across groups at equilibrium — the theoretical basis for using hit-rate divergence as a bias signal.</div></div><div style={{ borderLeft: "3px solid #39D98A", paddingLeft: 10, marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #24304622" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 6px", borderRadius: 4, background: "#39D98A22", color: "#39D98A", border: "1px solid #39D98A44" }}>CENSUS DATA</span><span style={{ fontSize: 11, fontWeight: 700, color: "#E8EDFB" }}>Office for National Statistics (2021)</span></div><div style={{ fontSize: 11, fontStyle: "italic", color: "#8896B3", marginBottom: 2 }}>Census 2021 — Ethnic group, England and Wales</div><div style={{ fontSize: 10, color: "#4A5568", marginBottom: 4 }}>ONS, released March 2023</div><div style={{ fontSize: 10, color: "#8896B3", lineHeight: 1.55, background: "#0D1525", borderRadius: 5, padding: "5px 8px" }}>Population shares by ethnic group used as the denominator in the stop_share vs pop_share comparison (Signal A).</div></div></div>
          </Card>
        )}
        ...
        {layer === "technical" && techTab === "kpt" && (
          <Card accent={C.gold}>
            <div style={{ fontWeight: 800, color: C.text, marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>Stage 3 — KPT Reallocation</div>
            <p style={{ margin: "0 0 10px", fontSize: 13, color: C.sub, lineHeight: 1.6 }}>Computes how many searches must be removed from each ROT-flagged group to bring its projected hit rate to the area average, then redistributes those searches to unflagged groups proportional to population share. Integer allocation via the largest-remainder method guarantees Σfinal = budget exactly.</p>
            <div style={{ background: "#0D1525", borderRadius: 8, padding: "10px 12px", border: `1px solid ${C.gold}33`, fontFamily: "monospace", fontSize: 12, color: C.text, lineHeight: 1.9, marginBottom: 12 }}>
              fair_stops(g) = hits(g) / area_avg_hr<br />
              excess(g)     = (stop_share(g) − fair_stop_share(g)) × budget<br />
              raw_quota(g)  = pop_proportional(g) − excess(g) <span style={{ color: C.muted }}>[if flagged]</span><br />
              raw_quota(g)  = pop_proportional(g) + gain(g) <span style={{ color: C.muted }}>[if unflagged]</span><br />
              floor_quota(g)= {pct(floorFrac, 0)} × pop_proportional(g)<br />
              final(g)      = max(raw_quota(g), floor_quota(g))
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
              <StatBox label="Pool freed (quota units)" value={fmt1(totalPool)} color={C.gold} small />
            ...
            <div style={{ marginTop: 10, fontSize: 11, color: C.muted }}>▲ Floor enforced — top-ups deducted from unflagged donor groups. Disp = (quota share) ÷ (pop share). Target: 1.00×.</div>
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: "1px solid #243046" }}><div style={{ fontSize: 10, color: "#8896B3", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 10 }}>Sources</div><div style={{ borderLeft: "3px solid #FFD166", paddingLeft: 10, marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #24304622" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 6px", borderRadius: 4, background: "#FFD16622", color: "#FFD166", border: "1px solid #FFD16644" }}>PRIMARY</span><span style={{ fontSize: 11, fontWeight: 700, color: "#E8EDFB" }}>Knowles, J., Persico, N. & Todd, P. (2001)</span></div><div style={{ fontSize: 11, fontStyle: "italic", color: "#8896B3", marginBottom: 2 }}>Racial bias in motor vehicle searches: Theory and evidence</div><div style={{ fontSize: 10, color: "#4A5568", marginBottom: 4 }}>Journal of Political Economy, 109(1), 203–229. DOI: 10.1086/319557</div><div style={{ fontSize: 10, color: "#8896B3", lineHeight: 1.55, background: "#0D1525", borderRadius: 5, padding: "5px 8px" }}>Derives the equilibrium result that hit rates equalise across groups absent discrimination. The fair_stops formula and excess quota calculation implement this directly.</div></div><div style={{ borderLeft: "3px solid #39D98A", paddingLeft: 10, marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #24304622" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 6px", borderRadius: 4, background: "#39D98A22", color: "#39D98A", border: "1px solid #39D98A44" }}>FLOOR PARAM</span><span style={{ fontSize: 11, fontWeight: 700, color: "#E8EDFB" }}>Home Office (2023)</span></div><div style={{ fontSize: 11, fontStyle: "italic", color: "#8896B3", marginBottom: 2 }}>Stop and search — Police powers</div><div style={{ fontSize: 10, color: "#4A5568", marginBottom: 4 }}>GOV.UK Statistics, published annually</div><div style={{ fontSize: 10, color: "#8896B3", lineHeight: 1.55, background: "#0D1525", borderRadius: 5, padding: "5px 8px" }}>Operational context for the floor fraction — no group falls below 80% of its population-proportional share, preventing extreme single-month reductions.</div></div><div style={{ borderLeft: "3px solid #8896B3", paddingLeft: 10, marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid #24304622" }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}><span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "1px 6px", borderRadius: 4, background: "#8896B322", color: "#8896B3", border: "1px solid #8896B344" }}>INTEGER ROUNDING</span><span style={{ fontSize: 11, fontWeight: 700, color: "#E8EDFB" }}>Hamilton, A. (1792)</span></div><div style={{ fontSize: 11, fontStyle: "italic", color: "#8896B3", marginBottom: 2 }}>Largest-remainder method (Hamilton method)</div><div style={{ fontSize: 10, color: "#4A5568", marginBottom: 4 }}>Apportionment method, US House of Representatives</div><div style={{ fontSize: 10, color: "#8896B3", lineHeight: 1.55, background: "#0D1525", borderRadius: 5, padding: "5px 8px" }}>Used to allocate integer quotas summing to exactly the total budget, preventing rounding drift.</div></div></div>
          </Card>
        )}
